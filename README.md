...
- **Text**: Volcengine Ark (Doubao-Seed-2.0-mini) via OpenAI-compatible `/chat/completions`.
...
from **Vercel project environment variables** and forwards to Ark. Keys are never
...
3. Set project environment variables:
   - `ARK_TEXT_API_KEY` — Volcengine Ark text (can share the image key)
...
> The proxy stays disabled until both token variables are configured. This prevents an accidental public relay from consuming provider quota.
>
...
#    .env.local
ARK_TEXT_API_KEY=...                                      # Volcengine Ark text (optional if same as image key)
ARK_IMAGE_API_KEY=...                                     # Volcengine Ark
...
- `doubao-seed-2-0-mini-260215` is a reasoning model → the app sets `response_format: json_object` and a generous `max_tokens`.
...
- Volcengine Ark supports browser CORS for the configured Ark endpoint; Mode B can be used with per-user keys. `vite preview`
...
- **Mode B** (checkbox) switches to direct browser calls with keys typed into the panel (stored in localStorage).
  Ark endpoints that allow CORS can be used with per-user keys; keys remain in each browser.
